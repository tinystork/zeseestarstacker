...
#   astap_timeout_sec       -> number: solve timeout (primary, fallback ansvr)
#   ansvr_timeout_sec       -> number: solve timeout (fallback)
...
class ZeSolverAdapter:
    ...
    def _build_request(self, v1, image_fits_path: str, fits_header, settings: dict):
        ...
        options_kwargs: dict[str, Any] = {
            ...
            # Default write policy: ZeSolver may overwrite the input.  The
            # adapter itself never writes — it consumes the returned canonical
            # WCS header and lets the caller write it back.
            "write_policy": v1.WritePolicy.OVERWRITE_INPUT,
        }
    ...
    def _convert_result(self, v1, result, fits_header, filename) -> SolverOutcome:
        ...
        # FAILED / SKIPPED_EXISTING_WCS / any unrecognised status -> FAILED.
    ...
    def _resolve_timeout(settings: dict) -> float:
        ...
        Primary source ``astap_timeout_sec``, fallback ``ansvr_timeout_sec``,
        default :data:`_DEFAULT_SOLVE_TIMEOUT_S`.
        """
        for key in ("astap_timeout_sec", "ansvr_timeout_sec"):
