...
# --- DANS LA CLASSE SeestarStackerGUI DANS seestar/gui/main_window.py ---
    ...
    def init_variables(self):
        ...
        # Display variables for localization of normalization and weighting methods
...
# --- DANS LA CLASSE SeestarStackerGUI DANS seestar/gui/main_window.py ---
    ...
    def create_layout(self):
        ...
        in_subframe = ttk.Frame(self.folders_frame); in_subframe.pack(fill=tk.X, padx=5, pady=(5, 2)); self.input_label = ttk.Label(in_subframe, text=self.tr("input_folder", default="Input:"), width=10, anchor="w"); self.input_label.pack(side=tk.LEFT); self.browse_input_button = ttk.Button(in_subframe, text=self.tr("browse_input_button", default="Browse..."), command=self.file_handler.browse_input, width=10); self.browse_input_button.pack(side=tk.RIGHT); self.input_entry = ttk.Entry(in_subframe, textvariable=self.input_path); self.input_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 5)); self.input_entry.bind("<FocusOut>", self._update_show_folders_button_state); self.input_entry.bind("<KeyRelease>", self._update_show_folders_button_state)
        out_subframe = ttk.Frame(self.folders_frame); out_subframe.pack(fill=tk.X, padx=5, pady=(2, 5)); self.output_label = ttk.Label(out_subframe, text=self.tr("output_folder", default="Output:"), width=10, anchor="w"); self.output_label.pack(side=tk.LEFT); self.browse_output_button = ttk.Button(out_subframe, text=self.tr("browse_output_button", default="Browse..."), command=self.file_handler.browse_output, width=10); self.browse_output_button.pack(side=tk.RIGHT); self.output_entry = ttk.Entry(out_subframe, textvariable=self.output_path); self.output_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 5))
        ...
        # Mapping between internal keys and displayed labels for normalization method
        self.norm_method_keys = ["none", "linear_fit", "sky_mean"]
        self.norm_key_to_label = {}
        self.norm_label_to_key = {}
        for k in self.norm_method_keys:
        ...
        # Mapping between internal keys and displayed labels for weighting method
        self.weight_method_keys = ["none", "noise_variance", "noise_fwhm"]
        self.weight_key_to_label = {}
        self.weight_label_to_key = {}
        for k in self.weight_method_keys:
        ...
        expert_content_frame_id = expert_scroll_canvas.create_window((0,0), window=expert_content_frame, anchor="nw", tags="expert_content_frame_tag_v2") 
        ...
        self.final_edge_crop_spinbox = ttk.Spinbox(self.crop_frame, from_=0.0, to=25.0, increment=0.5, width=6, format="%.1f", textvariable=self.final_edge_crop_percent_var); self.final_edge_crop_spinbox.pack(side=tk.LEFT, padx=2, pady=2) 
        ...
        self.output_format_frame = ttk.LabelFrame(expert_content_frame, text=self.tr("output_format_frame_title", default="Output FITS Format"), padding=5)
        ...
        self._update_feathering_options_state() 
        self._update_low_wht_mask_options_state() 
        print("DEBUG (GUI create_layout V_SaveAsFloat32_1): Fin création layout et appels _update_..._state.") # Version Log
    ...
    def _store_widget_references(self):
        ...
        for item_identifier, tooltip_translation_key in tooltips_config_list: 
            ...
            if isinstance(item_identifier, str):
                ...
                try: debug_item_name = f"WidgetDirect({widget_to_attach_tooltip.winfo_class()}-{id(widget_to_attach_tooltip)})"
                except: debug_item_name = f"WidgetDirect(id-{id(widget_to_attach_tooltip)})"
            else:
        ...
        print(f"DEBUG (GUI _store_widget_references V_SaveAsFloat32_1): Références et Tooltips stockés. Nb widgets trad: {len(self.widgets_to_translate)}, Nb tooltips créés dans cet appel: {tooltip_created_count}") # Version Log
    ...
    def update_ui_language(self):
        ...
        if hasattr(self, 'stack_norm_combo'):
            ...
            for k in self.norm_method_keys:
                ...
                self.norm_label_to_key[label] = k
            self.stack_norm_combo['values'] = list(self.norm_key_to_label.values())
            current_norm = self.stack_norm_method_var.get()
            self.stack_norm_display_var.set(self.norm_key_to_label.get(current_norm, current_norm))
        if hasattr(self, 'stack_weight_combo'):
            ...
            for k in self.weight_method_keys:
                ...
                self.weight_label_to_key[label] = k
            self.stack_weight_combo['values'] = list(self.weight_key_to_label.values())
            current_weight = self.stack_weight_method_var.get()
            self.stack_weight_display_var.set(self.weight_key_to_label.get(current_weight, current_weight))
...
# --- DANS LA CLASSE SeestarStackerGUI DANS seestar/gui/main_window.py ---
    ...
    def update_stretch_from_histogram(self, black_point_from_histo, white_point_from_histo):
        ...
        """
        print(f"DEBUG GUI (update_stretch_from_histogram V_HistoCallbackRefreshLight_1): Reçu BP_histo={black_point_from_histo:.4g}, WP_histo={white_point_from_histo:.4g}")

        data_source_for_histo_min = 0.0
        data_source_for_histo_max = 1.0
        
        is_histo_on_adu = False
        if hasattr(self, 'queued_stacker') and self.queued_stacker and \
           getattr(self.queued_stacker, 'save_final_as_float32', False) and \
           hasattr(self.queued_stacker, 'raw_adu_data_for_ui_histogram') and \
           self.queued_stacker.raw_adu_data_for_ui_histogram is not None:
            ...
            if hasattr(self.histogram_widget, 'data_min_for_current_plot') and self.histogram_widget.data_max_for_current_plot > 1.5:
                ...
                print(f"  -> Histogramme opérait sur données ADU. Plage histo: [{data_source_for_histo_min:.4g}, {data_source_for_histo_max:.4g}]")
        ...
        if range_histo_data < 1e-9: range_histo_data = 1.0
    ...
    def refresh_preview(self, recalculate_histogram=True):
        ...
        Version: RefreshHistoControl_1
        """
        print(f"[DEBUG-GUI] refresh_preview V_RefreshHistoControl_1: Called. Recalculate Histo: {recalculate_histogram}")
        ...
        if (self.current_preview_data is None or 
            ...
            if (not self.processing and self.input_path.get() and os.path.isdir(self.input_path.get())):
                ...
                if hasattr(self, 'preview_manager') and self.preview_manager:
                    self.preview_manager.clear_preview(self.tr('Select input/output folders.'))
                if hasattr(self, 'histogram_widget') and self.histogram_widget and recalculate_histogram: # Seulement si recalcul demandé
        ...
        except tk.TclError: print("  [RefreshPreview] Error getting preview parameters from UI."); return
        ...
        try:
            ...
            processed_pil_image, data_for_histogram_analysis = self.preview_manager.update_preview(
                self.current_preview_data, preview_params,
                stack_count=self.preview_img_count, total_images=self.preview_total_imgs,
                current_batch=self.preview_current_batch, total_batches=self.preview_total_batches
            )
            ...
            if recalculate_histogram: # Recalculer l'histogramme seulement si demandé
                if data_for_histogram_analysis is not None:
                    print(f"    [RefreshPreview] data_for_histogram_analysis - Shape: {data_for_histogram_analysis.shape}, Range: [{np.nanmin(data_for_histogram_analysis):.4g} - {np.nanmax(data_for_histogram_analysis):.4g}]")
                else:
            ...
            if hasattr(self, 'histogram_widget') and self.histogram_widget and recalculate_histogram:
                self.histogram_widget.plot_histogram(None)
        print("[DEBUG-GUI] refresh_preview V_RefreshHistoControl_1: Exiting.")
    ...
    def _debounce_refresh_preview(self, *args, recalculate_histogram=True): # Ajout argument
        ...
        try:
            ...
            self.debounce_timer_id = self.root.after(150, lambda rh=recalculate_histogram: self.refresh_preview(recalculate_histogram=rh))
        except tk.TclError: pass
...
###########################################################################################################################################
    ...
    def refresh_preview(self, recalculate_histogram=True): # <--- SIGNATURE CORRIGÉE ICI
        ...
        """
        print(f"[DEBUG-GUI] refresh_preview V_RefreshHistoControl_1_SignatureFix: Called. Recalculate Histo: {recalculate_histogram}")
...
#############################################################################################################################################
    ...
    def _processing_finished(self):
        ...
        if q_stacker is not None:
            ...
            aligned_count = getattr(q_stacker, 'aligned_files_count', 0); failed_align_count = getattr(q_stacker, 'failed_align_count', 0)
            failed_stack_count = getattr(q_stacker, 'failed_stack_count', 0); skipped_count = getattr(q_stacker, 'skipped_files_count', 0)
            processed_files_count = getattr(q_stacker, 'processed_files_count', 0); total_exposure = getattr(q_stacker, 'total_exposure_seconds', 0.0)
        ...
        if was_stopped_by_user: status_text_for_log = self.tr('processing_stopped')
            ...
            if "_mosaic" in os.path.basename(final_stack_path).lower() or "_reproject" in os.path.basename(final_stack_path).lower(): final_stack_type_for_summary = "Mosaïque Drizzle/Reproject"
            else: final_stack_type_for_summary = "Drizzle"
            status_text_for_log = self.tr(f"{final_stack_type_for_summary.lower()}_complete", default=f"{final_stack_type_for_summary} Complete")
        ...
        try:
            ...
            summary_lines.append(f"{self.tr('Images in Final Stack', default='Images in Final Stack')} ({final_stack_type_for_summary}): {images_stacked}")
...
# --- DANS LA CLASSE SeestarStackerGUI DANS seestar/gui/main_window.py ---
    ...
    def start_processing(self):
        ...
        print(f"  >>> CRITICAL GUI CHECK (JUSTE AVANT APPEL BACKEND): self.settings.save_final_as_float32 = {valeur_a_passer_pour_float32_gui} (type: {type(valeur_a_passer_pour_float32_gui)})")
        # === FIN AJOUT ===
