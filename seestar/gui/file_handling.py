...
class FileHandlingManager:
    ...
    def browse_input(self):
        ...
        last_path = (
            self.gui.settings.input_folder
            if hasattr(self.gui.settings, "input_folder")
            and self.gui.settings.input_folder
            else None
        )
        folder = filedialog.askdirectory(
            title=self.gui.tr("Select Input Folder"), initialdir=last_path
        )
        if folder:
            ...
            if hasattr(self.gui, "_update_show_folders_button_state"):
                self.gui.root.after_idle(
                    self.gui._update_show_folders_button_state
                )  # Use after_idle
    ...
    def browse_output(self):
        ...
        last_path = (
            self.gui.settings.output_folder
            if hasattr(self.gui.settings, "output_folder")
            and self.gui.settings.output_folder
            else None
        )
        folder = filedialog.askdirectory(
            title=self.gui.tr("Select Output Folder"), initialdir=last_path
        )
        if folder:
            abs_folder = os.path.abspath(folder)
    ...
    def browse_reference(self):
        ...
        if current_ref and os.path.isfile(current_ref):
            ...
            hasattr(self.gui.settings, "input_folder")
            and self.gui.settings.input_folder
            and os.path.isdir(self.gui.settings.input_folder)
        ):
    ...
    def browse_last_stack(self):
        file = filedialog.askopenfilename(
            title=self.gui.tr("Select previous stack"),
            filetypes=[("FITS files", "*.fit *.fits")],
            initialdir=self.gui.settings.output_folder or ".",
        )
        if file:
            p = os.path.abspath(file)
            self.gui.last_stack_path.set(p)
    ...
    def add_folder(self):
        ...
        folder = filedialog.askdirectory(
            title=self.gui.tr("Select Additional Images Folder"), initialdir=last_path
        )
        ...
        abs_input = (
            os.path.abspath(self.gui.input_path.get())
            if self.gui.input_path.get()
            else None
        )
        abs_output = (
            os.path.abspath(self.gui.output_path.get())
            if self.gui.output_path.get()
            else None
        )

        if abs_input and os.path.normcase(abs_folder) == os.path.normcase(abs_input):
            messagebox.showwarning(
                self.gui.tr("warning"), self.gui.tr("Input folder cannot be added")
            )
            return
        if abs_output:
            if os.path.normcase(abs_folder) == os.path.normcase(abs_output):
                messagebox.showwarning(
                    self.gui.tr("warning"), self.gui.tr("Output folder cannot be added")
                )
                return
            if os.path.normcase(abs_folder).startswith(
                ...
                messagebox.showwarning(
                    self.gui.tr("warning"),
                    self.gui.tr("Cannot add subfolder of output folder"),
                )
                return
        # --- Fin Validation ---
        ...
        if hasattr(self.gui, "handle_add_folder_request"):
            ...
            messagebox.showerror(
                self.gui.tr("error"), "Erreur interne lors de l'ajout du dossier."
            )
