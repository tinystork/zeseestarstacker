...
# --- DANS LA CLASSE SettingsManager DANS seestar/gui/settings.py ---
    ...
    def load_settings(self):
        ...
        # Obtenir d'abord un dictionnaire de valeurs par défaut propres
        default_values_dict = self.get_default_values()

        if not os.path.exists(self.settings_file):
            print(f"DEBUG (Settings load_settings): Fichier '{self.settings_file}' non trouvé. Application des valeurs par défaut.")
            self.reset_to_defaults() # Appliquer les défauts à l'instance self
            print(f"DEBUG (Settings load_settings): Tentative de sauvegarde du fichier settings avec les valeurs par défaut...")
            self.save_settings() # Créer le fichier avec les défauts
            # Logguer la valeur de la clé API après reset et avant de retourner
            print(f"DEBUG (Settings load_settings): Valeur astrometry_api_key après reset (fichier non trouvé): '{getattr(self, 'astrometry_api_key', 'ERREUR_ATTR')}'")
            return False
        ...
        try:
            ...
            # Au lieu de defaults = self.reset_to_defaults(), on utilise default_values_dict
            # Et on assigne directement aux attributs de self, en utilisant default_values_dict pour les fallbacks.

            # Processing
            self.input_folder = settings_data.get('input_folder', default_values_dict['input_folder'])
            self.output_folder = settings_data.get('output_folder', default_values_dict['output_folder'])
            self.reference_image_path = settings_data.get('reference_image_path', default_values_dict['reference_image_path'])
            self.bayer_pattern = settings_data.get('bayer_pattern', default_values_dict['bayer_pattern'])
            self.stacking_mode = settings_data.get('stacking_mode', default_values_dict['stacking_mode'])
            self.kappa = settings_data.get('kappa', default_values_dict['kappa'])
            self.batch_size = settings_data.get('batch_size', default_values_dict['batch_size'])
            self.correct_hot_pixels = settings_data.get('correct_hot_pixels', default_values_dict['correct_hot_pixels'])
            self.hot_pixel_threshold = settings_data.get('hot_pixel_threshold', default_values_dict['hot_pixel_threshold'])
            self.neighborhood_size = settings_data.get('neighborhood_size', default_values_dict['neighborhood_size'])
            self.cleanup_temp = settings_data.get('cleanup_temp', default_values_dict['cleanup_temp'])

            # Quality Weighting
            self.use_quality_weighting = settings_data.get('use_quality_weighting', default_values_dict['use_quality_weighting'])
            self.weight_by_snr = settings_data.get('weight_by_snr', default_values_dict['weight_by_snr'])
            self.weight_by_stars = settings_data.get('weight_by_stars', default_values_dict['weight_by_stars'])
            self.snr_exponent = settings_data.get('snr_exponent', default_values_dict['snr_exponent'])
            self.stars_exponent = settings_data.get('stars_exponent', default_values_dict['stars_exponent'])
            self.min_weight = settings_data.get('min_weight', default_values_dict['min_weight'])

            # Drizzle Settings
            self.use_drizzle = settings_data.get('use_drizzle', default_values_dict['use_drizzle'])
            self.drizzle_scale = settings_data.get('drizzle_scale', default_values_dict['drizzle_scale'])
            self.drizzle_wht_threshold = settings_data.get('drizzle_wht_threshold', default_values_dict['drizzle_wht_threshold'])
            self.drizzle_mode = settings_data.get('drizzle_mode', default_values_dict['drizzle_mode'])
            self.drizzle_kernel = settings_data.get('drizzle_kernel', default_values_dict['drizzle_kernel'])
            self.drizzle_pixfrac = settings_data.get('drizzle_pixfrac', default_values_dict['drizzle_pixfrac'])
            
            # Mosaic
            self.mosaic_mode_active = settings_data.get('mosaic_mode_active', default_values_dict['mosaic_mode_active'])
            loaded_mosaic_settings = settings_data.get('mosaic_settings', default_values_dict['mosaic_settings'])
            self.mosaic_settings = {
                'kernel': loaded_mosaic_settings.get('kernel', default_values_dict['mosaic_settings']['kernel']),
                'pixfrac': loaded_mosaic_settings.get('pixfrac', default_values_dict['mosaic_settings']['pixfrac'])
            }
            if self.mosaic_settings['kernel'] not in VALID_DRIZZLE_KERNELS:
                self.mosaic_settings['kernel'] = default_values_dict['mosaic_settings']['kernel']
            try:
                 pf = float(self.mosaic_settings['pixfrac'])
                 self.mosaic_settings['pixfrac'] = np.clip(pf, 0.01, 1.0)
            except (ValueError, TypeError):
                 self.mosaic_settings['pixfrac'] = default_values_dict['mosaic_settings']['pixfrac']

            # Chroma Correction
            self.apply_chroma_correction = bool(settings_data.get('apply_chroma_correction', default_values_dict['apply_chroma_correction']))
            print(f"DEBUG (Settings load_settings): apply_chroma_correction chargé: {self.apply_chroma_correction}")

            # SCNR Final
            self.apply_final_scnr = bool(settings_data.get('apply_final_scnr', default_values_dict['apply_final_scnr']))
            self.final_scnr_target_channel = str(settings_data.get('final_scnr_target_channel', default_values_dict['final_scnr_target_channel']))
            self.final_scnr_amount = float(settings_data.get('final_scnr_amount', default_values_dict['final_scnr_amount']))
            self.final_scnr_preserve_luminosity = bool(settings_data.get('final_scnr_preserve_luminosity', default_values_dict['final_scnr_preserve_luminosity']))
            ...
            self.bn_grid_size_str = str(settings_data.get('bn_grid_size_str', default_values_dict['bn_grid_size_str']))
            # ... (tous les autres settings expert de la même manière) ...
            self.cb_max_b_factor = float(settings_data.get('cb_max_b_factor', default_values_dict['cb_max_b_factor']))
            self.final_edge_crop_percent = float(settings_data.get('final_edge_crop_percent', default_values_dict['final_edge_crop_percent']))
            
            # Photutils BN
            print("DEBUG (Settings load_settings): Chargement des paramètres Photutils BN...")
            self.apply_photutils_bn = bool(settings_data.get('apply_photutils_bn', default_values_dict['apply_photutils_bn']))
            # ... (tous les autres settings photutils) ...
            self.photutils_bn_exclude_percentile = float(settings_data.get('photutils_bn_exclude_percentile', default_values_dict['photutils_bn_exclude_percentile']))
            print("DEBUG (Settings load_settings): Paramètres Photutils BN chargés.")
            
            # Astrometry API Key
            self.astrometry_api_key = str(settings_data.get('astrometry_api_key', default_values_dict['astrometry_api_key']))
            print(f"DEBUG (Settings load_settings): Clé API Astrometry chargée (longueur: {len(self.astrometry_api_key)}).")

            # Preview Settings
            self.preview_stretch_method = settings_data.get('preview_stretch_method', default_values_dict['preview_stretch_method'])
            # ... (tous les autres settings preview) ...
            self.preview_b_gain = settings_data.get('preview_b_gain', default_values_dict['preview_b_gain'])

            # UI Settings
            self.language = settings_data.get('language', default_values_dict['language'])
            self.window_geometry = settings_data.get('window_geometry', default_values_dict['window_geometry'])
            
            # --- NOUVEAU : Charger les paramètres de feathering ---
            # Pour ce test, on force apply_feathering à True et blur_px à une valeur de test
            # self.apply_feathering = bool(settings_data.get('apply_feathering', defaults_test['apply_feathering']))
            # self.feather_blur_px = int(settings_data.get('feather_blur_px', defaults_test['feather_blur_px']))
            self.apply_feathering = True  # FORCÉ pour le test
            self.feather_blur_px = 128    # FORCÉ pour le test (commencer bas)
            print(f"DEBUG (Settings load_settings): Feathering FORCÉ pour TEST -> Apply: {self.apply_feathering}, BlurPx: {self.feather_blur_px}")
            
            # --- FIN NOUVEAU --
            print(f"DEBUG (Settings load_settings): Paramètres chargés depuis '{self.settings_file}'. Validation en cours...")
            validation_messages = self.validate_settings() # Valider les settings chargés
            if validation_messages:
                 print("DEBUG (Settings load_settings): Paramètres chargés ajustés après validation:")
                 for msg in validation_messages: print(f"  - {msg}")
                 #self.save_settings() # Sauvegarder les settings corrigés
            return True

        except json.JSONDecodeError as e:
            print(f"Error decoding settings file {self.settings_file}: {e}. Using defaults and resetting file.")
            self.reset_to_defaults() # Réinitialiser l'instance aux valeurs par défaut
            self.save_settings()     # Écraser le fichier corrompu avec les valeurs par défaut
            return False
        except Exception as e: # Attrape d'autres erreurs comme des clés manquantes si pas de .get()
            print(f"Error loading settings from {self.settings_file}: {e}. Using defaults and resetting file.")
            traceback.print_exc(limit=2)
            self.reset_to_defaults() # Réinitialiser l'instance
            self.save_settings()     # Écraser le fichier avec les valeurs par défaut
            return False
