...
# --- END Replace _process_file in seestar/queuep/queue_manager.py ---
    ...
    def _stack_batch(self, batch_images, batch_headers, batch_scores, current_batch_num=0, total_batches_est=0):
        ...
        num_images = len(batch_images); progress_info = f"(Lot {current_batch_num}/{total_batches_est if total_batches_est > 0 else '?'})"
        ...
        try:
            ...
            if self.use_quality_weighting:
                ...
                if len(weights_1d) != num_images: self.update_progress("❌ Erreur interne: Nombre de poids != nombre d'images. Désactivation pondération pour ce lot."); weights_1d = None
                else: weight_shape = [-1] + [1] * (image_stack_np.ndim - 1); weights_broadcast = weights_1d.reshape(weight_shape)
            stacked_image = None
            with np.errstate(divide='ignore', invalid='ignore'):
                if self.stacking_mode == "mean":
                    ...
                    if weights_1d is not None: self.update_progress(f"⚠️ Pondération non supportée pour 'median', utilisation mediane simple {progress_info}.")
                    ...
                    if self.stacking_mode == "kappa-sigma":
                        lower_bound = mean - threshold; upper_bound = mean + threshold; mask = (image_stack_np >= lower_bound) & (image_stack_np <= upper_bound)
                        if weights_1d is not None:
                            masked_weights = np.where(mask, weights_broadcast, 0.0); sum_of_weights_kept = np.sum(masked_weights, axis=0)
                            sum_of_weights_kept = np.maximum(sum_of_weights_kept, 1e-9); weighted_sum_kept = np.sum(np.where(mask, image_stack_np * masked_weights, 0.0), axis=0)
                            stacked_image = weighted_sum_kept / sum_of_weights_kept
                        else:
                            count_kept = np.maximum(np.sum(mask, axis=0, dtype=np.int16), 1); sum_image = np.sum(np.where(mask, image_stack_np, 0.0), axis=0)
                            stacked_image = sum_image / count_kept
                        rejected_count = num_images * np.prod(common_shape) - np.sum(mask); rejected_percent = (rejected_count / (num_images * np.prod(common_shape))) * 100 if num_images > 0 else 0
                        ...
                        lower_bound = mean - threshold; upper_bound = mean + threshold; clipped_stack = np.clip(image_stack_np, lower_bound, upper_bound)
                        if weights_1d is not None: stacked_image = np.average(clipped_stack, axis=0, weights=weights_1d)
                        else: stacked_image = np.mean(clipped_stack, axis=0)
            ...
            stacked_image = np.nan_to_num(stacked_image).astype(common_dtype); stacked_image = np.clip(stacked_image, 0.0, 1.0)
            ...
            del image_stack_np; gc.collect(); return stacked_image, stack_info_header
        except Exception as e: self.update_progress(f"❌ Erreur empilement lot {progress_info}: {e}"); traceback.print_exc(limit=3); gc.collect(); return None, None
