...
def initialize_master(
    ...
    batch_img_f = np.asarray(batch_img, dtype=np.float32)
    batch_cov_f = np.asarray(batch_cov, dtype=np.float32)
    ...
    reproj_cov = np.asarray(reproj_cov, dtype=np.float32)
    ...
    master_sum = np.asarray(master_sum, dtype=np.float32)
    master_cov = np.asarray(master_cov, dtype=np.float32)
