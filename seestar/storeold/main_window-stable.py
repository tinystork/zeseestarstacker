...
# --- DANS LA CLASSE SeestarStackerGUI DANS seestar/gui/main_window.py ---
    ...
    def start_processing(self):
        ...
        2. Affiche l'avertissement Drizzle/Mosaïque si nécessaire.
        ...
        is_mosaic_mode_ui = getattr(self, 'mosaic_mode_active', False) # Mode Mosaïque (lu depuis le flag du GUI)

        if drizzle_globally_enabled_ui or is_mosaic_mode_ui:
            warning_title = self.tr('drizzle_warning_title')
            base_warning_text_tuple_or_str = self.tr('drizzle_warning_text') # Peut être un tuple ou str
            # Joindre si c'est un tuple, sinon utiliser directement si str
            base_warning_text = "".join(base_warning_text_tuple_or_str) if isinstance(base_warning_text_tuple_or_str, tuple) else base_warning_text_tuple_or_str
            ...
            print(f"DEBUG (GUI start_processing): Avertissement Drizzle/Mosaïque nécessaire. Texte: \"{full_warning_text[:70]}...\"")
        ...
        # ... (Identique à la version précédente que je vous ai donnée) ...
        ...
        folders_to_pass_to_backend = list(self.additional_folders_to_process); self.additional_folders_to_process = []
        self.update_additional_folders_display()
        ...
        print(f"  DEBUG GUI SETTINGS CHECK (après update_from_ui): self.settings.apply_feathering={getattr(self.settings, 'apply_feathering', 'NonTrouve')}, self.settings.feather_blur_px={getattr(self.settings, 'feather_blur_px', 'NonTrouve')}")
        print("  -> Appel self.settings.validate_settings()...")
        validation_messages = self.settings.validate_settings()
        print(f"  DEBUG GUI SETTINGS CHECK (après validate_settings): self.settings.apply_feathering={getattr(self.settings, 'apply_feathering', 'NonTrouve')}, self.settings.feather_blur_px={getattr(self.settings, 'feather_blur_px', 'NonTrouve')}")
        ...
        # ---- LOG DE TOUS LES PARAMETRES QUI SERONT PASSES AU BACKEND ----
        print(f"  GUI->BACKEND: input_folder='{self.settings.input_folder}'")
        print(f"  GUI->BACKEND: output_folder='{self.settings.output_folder}'")
        print(f"  GUI->BACKEND: reference_image_path='{self.settings.reference_image_path}'")
        print(f"  GUI->BACKEND: stacking_mode='{self.settings.stacking_mode}'")
        print(f"  GUI->BACKEND: kappa={self.settings.kappa}")
        print(f"  GUI->BACKEND: batch_size={self.settings.batch_size}")
        print(f"  GUI->BACKEND: correct_hot_pixels={self.settings.correct_hot_pixels}")
        print(f"  GUI->BACKEND: hot_pixel_threshold={self.settings.hot_pixel_threshold}")
        print(f"  GUI->BACKEND: neighborhood_size={self.settings.neighborhood_size}")
        print(f"  GUI->BACKEND: bayer_pattern='{self.settings.bayer_pattern}'")
        print(f"  GUI->BACKEND: cleanup_temp={self.settings.cleanup_temp}")
        print(f"  GUI->BACKEND: use_quality_weighting={self.settings.use_quality_weighting}")
        print(f"  GUI->BACKEND: weight_by_snr={self.settings.weight_by_snr}")
        print(f"  GUI->BACKEND: weight_by_stars={self.settings.weight_by_stars}")
        print(f"  GUI->BACKEND: snr_exponent={self.settings.snr_exponent}")
        print(f"  GUI->BACKEND: stars_exponent={self.settings.stars_exponent}")
        print(f"  GUI->BACKEND: min_weight={self.settings.min_weight}")
        print(f"  GUI->BACKEND: use_drizzle={self.settings.use_drizzle}")
        print(f"  GUI->BACKEND: drizzle_scale={float(self.settings.drizzle_scale)}")
        print(f"  GUI->BACKEND: drizzle_wht_threshold={self.settings.drizzle_wht_threshold}")
        print(f"  GUI->BACKEND: drizzle_mode='{self.settings.drizzle_mode}'")
        print(f"  GUI->BACKEND: drizzle_kernel='{self.settings.drizzle_kernel}'")
        print(f"  GUI->BACKEND: drizzle_pixfrac={self.settings.drizzle_pixfrac}")
        print(f"  GUI->BACKEND: apply_chroma_correction={self.settings.apply_chroma_correction}")
        print(f"  GUI->BACKEND: apply_final_scnr={self.settings.apply_final_scnr}")
        print(f"  GUI->BACKEND: final_scnr_target_channel='{self.settings.final_scnr_target_channel}'")
        print(f"  GUI->BACKEND: final_scnr_amount={self.settings.final_scnr_amount}")
        print(f"  GUI->BACKEND: final_scnr_preserve_luminosity={self.settings.final_scnr_preserve_luminosity}")
        print(f"  GUI->BACKEND: bn_grid_size_str='{self.settings.bn_grid_size_str}'")
        print(f"  GUI->BACKEND: bn_perc_low={self.settings.bn_perc_low}")
        print(f"  GUI->BACKEND: bn_perc_high={self.settings.bn_perc_high}")
        print(f"  GUI->BACKEND: bn_std_factor={self.settings.bn_std_factor}")
        print(f"  GUI->BACKEND: bn_min_gain={self.settings.bn_min_gain}")
        print(f"  GUI->BACKEND: bn_max_gain={self.settings.bn_max_gain}")
        print(f"  GUI->BACKEND: cb_border_size={self.settings.cb_border_size}")
        print(f"  GUI->BACKEND: cb_blur_radius={self.settings.cb_blur_radius}")
        print(f"  GUI->BACKEND: cb_min_b_factor={self.settings.cb_min_b_factor}")
        print(f"  GUI->BACKEND: cb_max_b_factor={self.settings.cb_max_b_factor}")
        print(f"  GUI->BACKEND: final_edge_crop_percent={self.settings.final_edge_crop_percent}")
        print(f"  GUI->BACKEND: apply_photutils_bn={self.settings.apply_photutils_bn}")
        print(f"  GUI->BACKEND: photutils_bn_box_size={self.settings.photutils_bn_box_size}")
        print(f"  GUI->BACKEND: photutils_bn_filter_size={self.settings.photutils_bn_filter_size}")
        print(f"  GUI->BACKEND: photutils_bn_sigma_clip={self.settings.photutils_bn_sigma_clip}")
        print(f"  GUI->BACKEND: photutils_bn_exclude_percentile={self.settings.photutils_bn_exclude_percentile}")
        print(f"  GUI->BACKEND: apply_feathering={self.settings.apply_feathering}") # Vérifier cette valeur
        print(f"  GUI->BACKEND: feather_blur_px={self.settings.feather_blur_px}")     # Vérifier cette valeur
        print(f"  GUI->BACKEND: is_mosaic_run={self.settings.mosaic_mode_active}") # Utiliser le flag de settings
        print(f"  GUI->BACKEND: api_key présent={'Oui' if self.settings.astrometry_api_key else 'Non'}")
        print(f"  GUI->BACKEND: mosaic_settings={self.settings.mosaic_settings}") # Utiliser les settings mosaïque de SettingsManager
        # ---- FIN LOG ----
        ...
        processing_started = self.queued_stacker.start_processing(
            input_dir=self.settings.input_folder, # Utiliser self.settings.X pour tous
            ...
            use_weighting=self.settings.use_quality_weighting, # Transmettre la valeur de settings
            weight_by_snr=self.settings.weight_by_snr,         # Transmettre la valeur de settings
            weight_by_stars=self.settings.weight_by_stars,     # Transmettre la valeur de settings
            snr_exp=self.settings.snr_exponent,                # Transmettre la valeur de settings
            stars_exp=self.settings.stars_exponent,            # Transmettre la valeur de settings
            min_w=self.settings.min_weight,                    # Transmettre la valeur de settings
            ...
            apply_feathering=self.settings.apply_feathering, # Utiliser self.settings.apply_feathering
            feather_blur_px=self.settings.feather_blur_px,     # Utiliser self.settings.feather_blur_px
            is_mosaic_run=self.settings.mosaic_mode_active, # Utiliser le flag de SettingsManager
            api_key=self.settings.astrometry_api_key,
            mosaic_settings=self.settings.mosaic_settings    # Utiliser les settings mosaïque de SettingsManager
        )
